...
import ph_transfo.configs
import ph_transfo.experiment
import ph_transfo.main
from ph_transfo.algorithms.no_op import NoOp
from ph_transfo.conftest import setup_with_overrides, skip_on_macOS_in_CI
from ph_transfo.utils.env_vars import REPO_ROOTDIR
from ph_transfo.utils.hydra_utils import resolve_dictconfig

logger = getLogger(__name__)

CONFIG_DIR = Path(ph_transfo.configs.__file__).parent
...
def test_experiment_config_is_tested(experiment_config: str, pytestconfig: pytest.Config):
    ...
    executing_subset_of_repo = any(
        "ph_transfo/" in arg for arg in pytestconfig.invocation_params.args
    )
...
def mock_train_and_evaluate(monkeypatch: pytest.MonkeyPatch):
    fn = ph_transfo.experiment.train_and_evaluate
    mock_train_fn = Mock(spec=fn, return_value=("fake", 0.0))
    monkeypatch.setattr(ph_transfo.experiment, fn.__name__, mock_train_fn)
    monkeypatch.setattr(ph_transfo.main, fn.__name__, mock_train_fn)
...
def test_can_load_experiment_configs(
    ...
    if dict_config["hydra"]["mode"] == RunMode.MULTIRUN:
        ...
        results = ph_transfo.main.main(dict_config)
...
def test_can_run_experiment(
    ...
    command_line_args = ["ph_transfo/main.py"] + list(command_line_overrides) + [f"name={name}"]
...
def test_help_string(file_regression: FileRegressionFixture) -> None:
    help_string = subprocess.run(
        ...
        shlex.split("python ph_transfo/main.py seed=123 --help"),
        text=True,
        capture_output=True,
    ).stdout
...
def test_run_auto_schema_via_cli_without_errors():
    ...
    # assuming we're at the ph_transfo root directory.
