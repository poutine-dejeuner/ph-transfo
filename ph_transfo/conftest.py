...
For example, when running `python ph_transfo/main.py algorithm=image_classifier`.
...
flowchart TD
datamodule_config[
    <a href="#ph_transfo.conftest.datamodule_config">datamodule_config</a>
] -- 'datamodule=A' --> command_line_arguments
algorithm_config[
    <a href="#ph_transfo.conftest.algorithm_config">algorithm_config</a>
] -- 'algorithm=B' --> command_line_arguments
command_line_overrides[
    <a href="#ph_transfo.conftest.command_line_overrides">command_line_overrides</a>
] -- 'seed=123' --> command_line_arguments
command_line_arguments[
    <a href="#ph_transfo.conftest.command_line_arguments">command_line_arguments</a>
] -- load configs for 'datamodule=A algorithm=B seed=123' --> dict_config
dict_config[
    <a href="#ph_transfo.conftest.dict_config">dict_config</a>
] -- instantiate objects from configs --> config
config[
    <a href="#ph_transfo.conftest.config">config</a>
] --> datamodule & algorithm
datamodule[
    <a href="#ph_transfo.conftest.datamodule">datamodule</a>
] --> algorithm
algorithm[
    <a href="#ph_transfo.conftest.algorithm">algorithm</a>
...
from ph_transfo.main import PROJECT_NAME, instantiate_algorithm, setup_logging
from ph_transfo.utils.env_vars import NUM_WORKERS, REPO_ROOTDIR
from ph_transfo.utils.hydra_utils import resolve_dictconfig
from ph_transfo.utils.testutils import (
    IN_GITHUB_CI,
    PARAM_WHEN_USED_MARK_NAME,
    default_marks_for_config_combinations,
    default_marks_for_config_name,
)
from ph_transfo.utils.typing_utils import is_sequence_of

if typing.TYPE_CHECKING:
    from _pytest.mark.structures import ParameterSet

    # TODO: Might not be present if the Jax examples aren't included.
    from ph_transfo.trainers.jax_trainer import JaxTrainer
...
def algorithm_config(request: pytest.FixtureRequest) -> str | None:
    ...
    included tests, for example as is done in [ph_transfo.algorithms.image_classifier_test][].
    """
...
def command_line_arguments(
    ...
    indirectly by most tests using the [`ph_transfo.utils.testutils.run_for_all_configs_of_type`][]
...
def dict_config(
    ...
    logger.info(
        ...
        + "\n"
        + "```\n"
        + ("python ph_transfo/main.py " + " ".join(command_line_arguments) + "\n")
        + "```\n"
    )
...
def setup_with_overrides(
    ...
    def test_something(dict_config: omegaconf.DictConfig):
        ...
        assert dict_config["algorithm"]["_target_"] == "ph_transfo.algorithm.image_classifier.ImageClassifier"
